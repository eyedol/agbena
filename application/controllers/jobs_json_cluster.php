...
class Jobs_Json_Cluster_Controller extends Template_Controller
{
	...
	public function index()
    {
		...
		$filter = "";
		$filter .= ($category_id !=0) ? " AND ( j_category.id=".$category_id
			." OR j_category.parent_id=".$category_id.") " : "";
		$filter .= ($start_date) ? 
			" AND job.job_dateadd >= '" . date("Y-m-d H:i:s", $start_date) . "'" : "";
		$filter .= ($end_date) ? 
			" AND job.job_dateadd <= '" . date("Y-m-d H:i:s", $end_date) . "'" : "";
		...
		if ($category_id > 0)
		{
			$query_cat = $db->query("SELECT `job_category_color`, `job_category_image` FROM `j_category` WHERE id=$category_id");
			foreach ($query_cat as $cat)
			{
				$color = $cat->job_category_color;
				$icon = $cat->job_category_image;
			}
		}
...
//			->find_all();
		...
		$query = $db->query("SELECT DISTINCT `job`.id, `location`.`latitude`, `location`.`longitude` FROM `job` INNER JOIN `location` ON (`location`.`id` = `job`.`location_id`) INNER JOIN `job_category` ON (`job`.`id` = `job_category`.`job_id`) INNER JOIN `j_category` ON (`job_category`.`j_category_id` = `j_category`.`id`) WHERE job.job_active=1 $filter ORDER BY `job`.`id` ASC ");	
...
//		echo count($query);
	...
	public function single($job_id = 0)
	{
		$json = "";
		$json_item = "";
		$json_array = array();	
		...
		if ($marker->loaded)
		{
			...
			$job_date = date('Y-m', strtotime($marker->job_date));
			$latitude = $marker->location->latitude;
			$longitude = $marker->location->longitude;
			
			$filter = " AND job.job_date LIKE '$job_date%' ";
			$filter .= " AND job.id <>".$marker->id;
			
			// Database
			$db = new Database();
			
			// Get Neighboring Markers From The Same Month Within A Mile
			$query = $db->query("SELECT DISTINCT `job`.*, `location`.`latitude`, `location`.`longitude`, 
			((ACOS(SIN($latitude * PI() / 180) * SIN(`location`.`latitude` * PI() / 180) + COS($latitude * PI() / 180) * COS(`location`.`latitude` * PI() / 180) * COS(($longitude - `location`.`longitude`) * PI() / 180)) * 180 / PI()) * 60 * 1.1515) AS distance 
			 FROM `job` INNER JOIN `location` ON (`location`.`id` = `job`.`location_id`) INNER JOIN `job_category` ON (`job`.`id` = `job_category`.`job_id`) INNER JOIN `category` ON (`job_category`.`category_id` = `category`.`id`) WHERE job.job_active=1 $filter 
			HAVING distance<='1'
			 ORDER BY `job`.`id` ASC ");
			...
			foreach ($query as $row)
			{
	            ...
	            $json_item .= "\"properties\": {";
				$json_item .= "\"id\": \"".$row->id."\", ";
	            $json_item .= "\"name\":\"" . str_replace(chr(10), ' ', str_replace(chr(13), ' ', "<a href='" . url::base() . "reports/view/" . $row->id . "'>" . htmlentities($row->job_title) . "</a>")) . "\",";
				$json_item .= "\"category\":[0], ";
	            $json_item .= "\"timestamp\": \"" . strtotime($row->job_date) . "\"";
	            $json_item .= "},";
	            $json_item .= "\"geometry\": {";
	            $json_item .= "\"type\":\"Point\", ";
            ...
            $json_item .= "\"name\":\"" . str_replace(chr(10), ' ', str_replace(chr(13), ' ', "<a href='" . url::base() . "reports/view/" . $marker->id . "'>" . htmlentities($marker->job_title) . "</a>")) . "\",";
			$json_item .= "\"category\":[0], ";
            $json_item .= "\"timestamp\": \"" . strtotime($marker->job_date) . "\"";
            $json_item .= "},";
            $json_item .= "\"geometry\": {";
            $json_item .= "\"type\":\"Point\", ";
	...
	public function timeline()
	{
        ...
        // get graph data
        $graph_data = array();
        $all_graphs = Job_Model::get_jobs_by_interval($interval,$start_date,$end_date,$active);
	    echo $all_graphs;
   	}
