 ...
 */
class Jobsmain_Controller extends Template_Controller {
    ...
    {		
				...
				->find_all() as $category)
        {
			...
			foreach ($category->children as $child)
			{
				$children[$child->id] = array( 
					$child->job_category_title, 
					$child->job_category_color
				);
			}
        ...
        $this->template->content->total_items = ORM::factory('job')
            ->where('job_active', '1')
            ->limit('8')->count_all();
        $this->template->content->incidents = ORM::factory('job')
            ->where('job_active', '1')
			->limit('10')
